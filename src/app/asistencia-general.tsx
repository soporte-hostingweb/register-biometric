...
export default function GeneralAttendanceScreen() {
  ...
  return <SafeAreaView style={s.page} edges={['top']}>
    ...
    <ScrollView contentContainerStyle={s.content}>
      ...
      {!loading && !selectedId && <View style={s.empty}><Ionicons name="people-outline" size={46} color="#65B7F5" /><Text style={s.emptyTitle}>{tr('Choose an employee', 'Elige un empleado')}</Text><Text style={s.muted}>{tr('Records are not mixed: only the selected employee will be displayed.', 'Los registros no se mezclarán: solo se mostrará el empleado seleccionado.')}</Text></View>}
      {employee && <>
